...
def fit(num_epochs,model,trainloader,testloader,i):
    ...
    for epoch in range(num_epochs):
        ...
        if  epoch_loss_test < min_loss:
            ...
            if i == 0:
                torch.save(state, os.path.join(base_dir ,'only_mr',"survive_cnn_min_loss_only_mr_TCGA.pth"))
            elif i == 1:
                torch.save(state, os.path.join(base_dir ,'only_mr',"survive_cnn_min_loss_only_mr_huashan.pth"))
        # if  epoch % 10 == 0:
        #     state = {
        #           'state_dict': model.state_dict(),
        #           'optimizer' : optim.state_dict()
        #         }
        #     torch.save(state, base_dir +"gbm_survive_cnn_min_loss_only_"+ str(epoch)+ ".pth")
...
if __name__ == "__main__":
    ...
    train_ds = T1Dataset_regression(df_train,base_dir[i])
    test_ds = T1Dataset_regression(df_test,base_dir[i])
    train_dl = DataLoader(
                        train_ds,
                        shuffle= False,
                        batch_size = 20,
